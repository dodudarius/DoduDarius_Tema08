    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)
        {
            if (lightON == true && lightON_1 == true)
            {
                switch (e.KeyCode)
                {
                    case Keys.A: // Deplasare X negativ
                        valuesPosition1[0] -= MOVE_SPEED;
                        break;
                    case Keys.D: // Deplasare X pozitiv
                        valuesPosition1[0] += MOVE_SPEED;
                        break;
                    case Keys.W: // Deplasare Z negativ
                        valuesPosition1[2] -= MOVE_SPEED;
                        break;
                    case Keys.S: // Deplasare Z pozitiv
                        valuesPosition1[2] += MOVE_SPEED;
                        break;
                    case Keys.Q: // Deplasare Y negativ
                        valuesPosition1[1] -= MOVE_SPEED;
                        break;
                    case Keys.E: // Deplasare Y pozitiv
                        valuesPosition1[1] += MOVE_SPEED;
                        break;
                }
        ...
        private void setControlMouseLight1(bool status)
        {
            if (status == false)
            {
                statusControlMouseLight1 = false;
                //btnMouseControlLight1.Text = "Light1 mouse control OFF";
            }
            else
            {
                statusControlMouseLight1 = true;
                // btnMouseControlLight1.Text = "Light1 mouse control ON";
            }
        }
        ...
        private void GlControl1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                if (lightON == true && lightON_1 == true && statusControlMouseLight1 == true)
                {
                    float deltaX = e.X - mousePos.X;
                    float deltaY = e.Y - mousePos.Y;

                    valuesPosition1[0] += deltaX * 0.1f;
                    valuesPosition1[1] -= deltaY * 0.1f;
        ...
        // NOU: Activăm/dezactivăm sursa 1 de iluminare.
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e) {
            ...
            if (statusControlMouse2D == true && statusControlMouseLight1 == false)
            {
                GL.Rotate(mousePos.X, 0, 1, 0);
            }
            else if (statusControlMouse3D == true && statusControlMouseLight1 == false)
            {
